...
- filenames use the existing timestamp, title sanitisation, and `(2)` collision rules,
  produced by the same extracted `uniquePath` rather than a separate mobile-side
  implementation;
...
6. Before retrying after an interrupted delivery, compare the intended filename and exact
   ...
   This content-hash approach reuses `own-writes.ts` (B31), which already tells the app's
   own pending writes apart from real external changes by content hash rather than a timer,
   because OneDrive's own re-materialisation schedule is not a clock this app can trust.
   Port that hashing logic into the Swift bridge rather than reimplementing it.
   ...
   `conflicts.ts` deliberately avoids a bare `(N)` suffix when detecting OneDrive conflict
   copies, because it would collide with `uniquePath`'s own numbering — a mobile-side
   collision suffix that doesn't match `uniquePath` exactly would either collide with
   desktop's own numbering or falsely trip that conflict-copy detector once the file
   reaches the desktop.
...
Measuring on real hardware before committing to a design is deliberate: the desktop's own
`awaitWriteFinish: 300ms` production default has never been validated against real OneDrive
sync latency end-to-end, and OneDrive-dependent behaviour built on reasoned-not-measured
assumptions has been costly before. Phase 0 exists to avoid repeating that on new ground.
