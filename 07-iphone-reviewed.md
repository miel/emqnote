...
> **Review, 2026-08-20.** This is `07-iphone.md` checked against the current codebase and
> `05-besluitenlog.md`/`CONSTRAINTS.md`/`TODO.md`. Most claims held up unchanged. Two
> changes are substantive rather than corrective: §1 now names and rejects the lighter
> Shortcut-based alternative B53 itself suggested trying first, and §5 now flags a real
> filename-collision landmine between this plan's collision handling and `conflicts.ts`'s
> existing OneDrive-conflict-copy detection. The rest are precision fixes — which parts of
> `capture-store.ts`/`filename.ts` are actually pure and extractable, a missing sibling file
> (`note-files.ts`), a corrected "DOM-free" claim about `schema.ts`, a note on how the new
> package must declare its dependencies, two small factual additions to the Task button
> section, and tying Phase 0 to this project's track record on unmeasured OneDrive latency
> assumptions — marked inline where they occur.
...
B53's own text names a lighter option for that open question: a one-hour iOS Shortcut that
writes a correctly formed `.md` file directly into the Inbox (06-ipad.md's route D),
calling it "the first thing to try" for capture, not an app. This plan deliberately goes
further than that. A Shortcut cannot host the Task/Tag buttons' caret and selection rules,
cannot keep a durable offline outbox with idempotent retry, and cannot give dictation a
structured place to land (title, When/Where/Who, and body as distinct fields rather than
one text blob). Those three needs are what justify a real app instead of the smaller thing
B53 flagged as worth trying first.
...
- On an ordered-list item, `toggleTask` first converts the list to a bullet list before
  applying the task toggle — the same behaviour the desktop editor already has.
- Return after a task creates another unchecked task. Return on an empty task leaves the
  task list, following the existing editor behaviour. This is handled by separate Enter-key
  keymap logic (`exitList`), not by `toggleTask` itself — the mobile editor needs to reuse
  both, not `toggleTask` alone.
...
- filenames use the existing timestamp, title sanitisation, and `(2)` collision rules —
  produced by the same extracted `uniquePath`, not a separate mobile-side implementation
  (see the landmine noted in §5);
...
`src/markdown/schema.ts` is Electron-free, but it is not DOM-free: its `toDOM`/`parseDOM`
specs call real DOM APIs (`getAttribute`, `querySelector`, `closest`), because it is also
the editor schema. This is not a blocker — a Capacitor WKWebView provides a real DOM, the
same way the desktop's Chromium renderer does — but the schema needs a DOM implementation
of some kind to load, and should not be assumed to run in a DOM-less test harness.
...
│           ├── note-files.ts    # noteStem/noteExtension — filename.ts's uniquePath needs it
│           └── time.ts
...
The root `package.json`'s "keep `dependencies` minimal, everything else in
`devDependencies`" rule is specific to electron-vite, which bundles everything not listed
there; it does not transfer to `packages/core/package.json`. The new package needs `react`
and the `prosemirror-*` packages declared as real dependencies — as `peerDependencies`
specifically, so the workspace resolves a single shared instance of each. ProseMirror's
`instanceof`-based type checks are known to break across two separate copies of the same
package, which a plain `dependencies` declaration risks if npm hoists differently for
`apps/iphone` than for the existing desktop build.
...
1. Extract `buildFrontmatter` and `firstLineOf` from `src/main/capture-store.ts` into
   `packages/core/src/capture/` — these two functions (and the `mergeTags`/`bodyTagsOf`
   call inside `buildFrontmatter`) are the actually fs-free part of that file.
   `writeSession`, `CaptureWriter`, and `renameSessionFile` are Node/session-lifecycle code
   and stay behind (or get a Swift-bridge equivalent); they are not part of this move.
   Because `vault-io.ts`'s `saveNote` and `capture-store.ts`'s `buildFrontmatter` are
   required by their own code comments to stay behaviourally identical (B65), desktop
   should import the moved `buildFrontmatter` rather than keep a parallel copy, with a
   shared test enforcing that the two never drift.
2. Split timestamp and filename sanitisation from the Node filesystem collision check in
   `src/main/filename.ts`, moving the pure rules into `packages/core/src/filename.ts`. Move
   `src/main/note-files.ts` (`noteStem`/`noteExtension`) alongside it — it is itself pure
   but `uniquePath` depends on it, and it currently lives in `src/main/` rather than
   anywhere shared.
...
5. Reuse the existing schema, checkbox rendering, `toggleTask`, `exitList`'s Enter-key
   handling, tag extraction, and serializer. Add a small tested `insertTagPrefix` editor
   command for the Tag button.
...
6. Before retrying after an interrupted delivery, compare the intended filename and exact
   ...
   This content-hash-over-timer approach is not new: `own-writes.ts` (B31) already tells
   the app's own pending writes apart from real external changes by content hash rather
   than a timer, explicitly because "OneDrive's own re-materialisation schedule is the one
   clock this app cannot trust." Port that hashing logic rather than reimplementing it in
   the Swift bridge.

   The collision-safe name in step 6, and the `(2)` rule in §3, must come from the same
   extracted `uniquePath` the desktop uses — not a separate naming scheme invented for iOS.
   `CONSTRAINTS.md` records that `conflicts.ts` deliberately avoids a bare `(N)` suffix when
   detecting OneDrive conflict copies specifically because it would collide with
   `uniquePath`'s own numbering. A mobile-side collision suffix that doesn't match
   `uniquePath` exactly risks either colliding with desktop's own numbering or falsely
   tripping `conflicts.ts`'s conflict-copy detector when the file reaches the desktop.
...
Measuring on real hardware before committing to a design is deliberate, not caution for its
own sake: `TODO.md` records that the desktop's own `awaitWriteFinish: 300ms` production
default has never been validated against real OneDrive sync latency end-to-end, and this
project has repeatedly shipped OneDrive-dependent behaviour on reasoned-not-measured
assumptions. Phase 0 exists so this plan doesn't repeat that pattern on new ground.
...
8. A delivered file appears in `00 Inbox/`, is parsed without repair by the desktop app,
   and exposes its open tasks and tag immediately — measured end-to-end (iPhone write →
   OneDrive → desktop watcher visible), not just confirmed to eventually arrive.
