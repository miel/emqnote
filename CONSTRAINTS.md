...
**Three MSAL settings on the iPhone app fail silently when they are missing, and each fails with a symptom that does not name its cause** (B77). *The Microsoft sign-in callback goes in `SceneDelegate.swift`, not `AppDelegate.swift`* — the Capacitor project has a scene delegate, so `application(_:open:options:)` is never called at all, and virtually every MSAL iOS sample shows exactly that form; the symptom is a sign-in that opens Safari, completes, and never returns to the app, with nothing logged anywhere. `scene(_:openURLContexts:)` calls `MSALPublicClientApplication.handleMSALResponse` and then still forwards to `SceneDelegateProxy.shared`, because MSAL claims only its own redirect and Capacitor needs the rest. *`Info.plist`'s `LSApplicationQueriesSchemes` must list `msauthv2` and `msauthv3`* — without them MSAL cannot detect the Microsoft Authenticator broker and quietly falls back to an in-app webview, so a Conditional Access policy demanding an approved client app fails while reporting a problem with the app rather than with the policy. *`App.entitlements` must carry the keychain access group `com.microsoft.adalcache`* — without it the interactive sign-in succeeds and `acquireTokenSilent` then never finds an account again, so every single launch asks for a fresh sign-in and nothing anywhere says why. Adjacent to all three: **MSAL 2.15 requires iOS 17**, which is why `IPHONEOS_DEPLOYMENT_TARGET` moved from 15.0; building at 15.0 produces a linker warning rather than an error, which is exactly the kind of thing that ships.

**The iPhone's Graph delivery never asks Graph to replace anything, and the collision guard is one query parameter whose behaviour has to be measured** (B77, `09-iphone-graph.md` §4). The upload is `PUT /drives/{driveId}/items/{inboxId}:/{name}:/content?@microsoft.graph.conflictBehavior=fail`, expecting `409 nameAlreadyExists`. Microsoft documents that annotation for *upload sessions*; the query-string form on simple upload is widely used and widely asserted, which is not the same as measured, and if it is wrong the failure mode is the silent replacement of somebody's note. The fallback is `createUploadSession` with the annotation in the body — one extra round trip on a file of a few kilobytes. `graph-results.md`'s matrix has the row that decides it, and until that row is filled in nothing should be built on top of the current form. Never `conflictBehavior=replace`, and never a name Graph chose: a server-side rename does not follow the product-wide `(2)` contract, and the desktop reads filenames.

**The iPhone's collision suffix comes from `collisionCandidate` in `packages/core/src/filename.ts`, the same function `uniquePath` walks** (B78). Not tidiness. `conflicts.ts` deliberately refuses to treat a bare ` (N)` suffix as a OneDrive conflict copy *because* it is `uniquePath`'s own shape, so a numbering scheme invented for iOS would either collide with the desktop's names or make the desktop mistake a perfectly ordinary second note for a conflict copy. `uniquePath` asks a synchronous `exists` that a remote destination cannot answer, which is why the sequence was extracted rather than reused whole: the desktop walks it against the filesystem, the iPhone walks it one Graph round trip at a time, and `test/filename.test.ts` pins that the two agree about what the fifth name is.

**Deciding whether an interrupted iPhone upload landed is done by downloading the file and comparing bytes, never by a hash Graph publishes** (B79). Business OneDrive exposes only `quickXorHash` in `file.hashes` — no sha256, no sha1 — so the obvious route is to implement QuickXorHash in Swift. Don't. That puts a hand-written, untested hash on precisely the one decision this app must not guess at, and a bug in it produces no error at all: just a silent wrong answer that is either a duplicated note or an overwritten one. `probeItem` fetches the content and hashes it; `delivery/graph.ts` compares against a sha256 the JavaScript side computes over the same UTF-8 bytes. **The `TextEncoder` / `Data(text.utf8)` pairing is the whole contract** — change one without the other and every comparison silently becomes "different bytes", which the delivery layer reads as somebody else's note and politely renames around, duplicating the note it was trying not to duplicate. When the comparison itself fails there is no honest answer and the item is retried, not guessed at.

**The iPhone's delivery policy lives in TypeScript and the Swift bridges decide nothing** (B78). Which item is next, when to retry, what the next name is, whether an interrupted upload counts — all of that is `apps/iphone/src/delivery/`, pure where it can be and covered by `npm run test:iphone` on any machine. Swift does only what needs an iOS API: a token in the Keychain, the broker, an HTTPS request outside a webview's CORS rules, a coordinated file write. The reason is arithmetic rather than taste: the Xcode project has one target and no XCTest target, so every line moved into Swift is a line whose only instrument is the probe panel and whose only record is an evidence sheet. Interactive sign-in in particular is never triggered from a retry — a background drain that pops a Microsoft sign-in sheet puts it in front of someone who opened the app to type one line, from a queue they had forgotten about.
