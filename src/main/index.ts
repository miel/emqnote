...
// PDF/Office thumbnail is generated by the OS, cached as a PNG under `userData`, and
// served the same way — never as a `data:` URL, which would mean pushing a full base64
// image through IPC on every `setDoc` (every note-open) and reopening the CSP hole B28
// closed for attachment previews generally. See thumbnails.ts / thumbnail-cache.ts.
...
/**
 * Serves a first-page PDF/Office thumbnail as `emqnote-thumb://<name>` (B30). Reuses
 * `resolveAttachment` verbatim for the traversal guard — this is the same `_attachments/`
 * a name has to resolve inside, just a different rendering of the same file — and adds
 * `isPreviewable` on top, since a plain `[[Some Note]]` link or a `.txt` attachment must
 * 404 without ever reaching `nativeImage` at all.
 *
 * The `<img>` element in `attachment-view.ts` is the whole state machine here: no IPC
 * round trip, no "ask main, wait, then set src" — a 404 fires the tag's own `onerror` and
 * the NodeView falls back to the plain chip.
 */
function registerThumbnailProtocol(): void {
  protocol.handle("emqnote-thumb", async (request) => {
    ...
    const file = await ensureThumbnail(thumbnailCacheDir(), resolved);
