...
describe("finding orphaned attachments", () => {
  it("finds an attachment nothing refers to", async () => {
    ...
    expect(await findOrphanedAttachments(vault)).toEqual(["_attachments/2026/07/afbeelding-1.png"]);
  });
  ...
  it("does not flag an attachment an embed points at", async () => {
    ...
    expect(await findOrphanedAttachments(vault)).toEqual([]);
  });
  ...
  it("finds only the attachments nothing refers to, among several", async () => {
    ...
    expect(await findOrphanedAttachments(vault)).toEqual(["_attachments/2026/07/ongebruikt.png"]);
  });
...
describe("finding them from the index's targets", () => {
  it("takes the reference set it is given instead of reading the notes", async () => {
    ...
    expect(await findOrphanedAttachments(vault, ["gebruikt.png"])).toEqual([
  ...
  it("does not descend into the folders the app owns when it falls back to reading", async () => {
    ...
    expect(await findOrphanedAttachments(vault)).toEqual(["_attachments/2026/07/plaatje.png"]);
  });
});
